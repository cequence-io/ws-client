...
object PlayWSMultipartWritable {
  ...
  /**
   * `Writeable` for `MultipartFormData` using streaming (SourceBody).
   * Efficient for large files but uses chunked transfer encoding,
   * which some servers (e.g. behind Cloudflare) may reject.
   */
  ...
  /**
   * `Writeable` for `MultipartFormData` using in-memory body (InMemoryBody).
   * Materializes the entire body upfront, avoiding chunked transfer encoding.
   * Use this when the target server does not support chunked requests.
   */
  def writeableOf_MultipartFormDataInMemory(
    ...
    BodyWritable[MultipartFormData](
      transform = { (form: MultipartFormData) =>
        ...
        val fileParts = form.files.foldLeft(ByteString.empty) { (acc, file) =>
